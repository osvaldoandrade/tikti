...
The current system is a Firebase‑compatible API surface: `signIn`, `signInWithPassword`, `lookup`, `update`, `delete`, and OOB password reset. Tokens are HS256 signed using a shared secret. User records are stored in Redis keyed by email, and out‑of‑band codes are stored in a separate hash with expiration enforced at read time. These behaviors are preserved for compatibility, but Tikti is extended to include issuer and audience claims, a token exchange endpoint for RS256 tokens, and multi‑tenant membership and role evaluation.
