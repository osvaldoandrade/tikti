...
non-empty `X-API-Key` header plus a strict RS256 Bearer for the configured
issuer and Code Admin audience; query-string credentials and legacy HS256
tokens are not accepted. Tenant-local GET accepts `code-admin:identity:read`
or `code-admin:identity:write`, PUT/DELETE require
`code-admin:identity:write`, and signed `tid` must exactly match the path.
Cross-tenant access requires provenance-bound platform-admin authority.
...
The access token must be RS256-signed and contain a non-empty subject, the
`code-admin:tenants:admin` scope, the persisted `ADMIN` role, and
