...
The `code-admin:` namespace is reserved. Tikti embeds Code Admin scope policy `2026-09-08.1` with SHA-256 `c3d6339018d16317bd8e356a2983930c4a0372d9666a68df126619be8def33db`; only entries explicitly marked tenant-assignable may appear in tenant roles. Unknown, global, mixed, and tenant-bound but non-assignable Code Admin scopes fail closed during role writes, exact reads, and strict token expansion. Permissions outside `code-admin:` retain the existing syntax contract because Tikti does not yet have a complete offline registry for those namespaces. Environment reads are tenant-assignable because Code Admin projects the globally stored inventory through the signed tenant before pagination, and repository reads are tenant-assignable because tenant repositories remain exact-tenant while the shared template catalog is immutable. Writes and control-plane administration remain non-assignable. The tenant-bound Secrets scopes are assignable because the Code Admin API keeps the routes feature-gated and the Console remains dark unless the existing controller-service owner renews the complete-runtime lease.
...
Automatically generated creator roles and their memberships are immutable. A creator role produced before Tenant Secrets therefore cannot be rewritten to add `code-admin:secrets:read` or `code-admin:secrets:write`. During a Code Admin tenant-target exchange, Tikti recognizes only the exact `<tenantId>-admin` role whose own persisted definition contains `code-admin:identity:write`, and adds only those two Tenant Secret scopes to its authority. Both scopes must still be present in the immutable managed-client defaults and in the server-supplied request ceiling. Unrelated roles, similarly named roles, and creator roles without identity administration receive no compatibility authority. Rollback is the previous Tikti image and does not mutate roles, memberships, or clients.
...
An installation that predates the MASTER tenant has one bounded recovery path. When an active password user retains the persisted `ADMIN` role and a non-MASTER legacy home, dynamic discovery selects `local-tenant` as the identity principal only if that exact user has a direct `ADMIN` access assignment in MASTER and the managed MASTER audience and role resolve successfully. The MASTER token retains its tenant `roles` evidence and adds the platform `role`, scope, and issuance marker required by global consumers. Tikti revalidates the direct assignment, current role set, tenant, and managed client whenever the token is used. A group-inherited `ADMIN`, a `COMPANY_ADMIN`, an inactive account, a similarly named role, or removal of the direct assignment cannot retain or recreate platform authority. Rolling back the issuer restores denial and does not mutate the preserved workload credential or either tenant assignment.

When `tenantTargetDiscoveryV2` is enabled, an exchanged tenant-local access token may read exact memberships with `code-admin:identity:read` or `code-admin:identity:write`, and may write memberships with `code-admin:identity:write`, only when its signed `tid` exactly matches the route tenant. This permits a creator who has switched into a dynamically discovered tenant to administer that tenant without adding its name to the v1 canary lists. The local path never accepts `code-admin:tenants:admin` as a substitute, never permits a cross-tenant target, and remains disabled with the v2 flag off. No data migration is required. Roll back by disabling dynamic tenant discovery and restoring the previous Tikti image; existing memberships and roles remain intact for a corrected replay.

Managed `code-admin-api` audience reconciliation includes one bounded legacy migration. A markerless client may be adopted only when its identifier and tenant match the reserved target, it is credential-free, active, `SERVICE`, uses only `token_exchange`, stores canonical scopes, and every stored scope is inside the current server-selected ceiling. Adoption uses compare-and-set semantics and atomically replaces the legacy value with identical managed value and ownership marker records. Any credential, unknown field, broader scope, alternate grant, tenant mismatch, or concurrent change remains a conflict. Rolling back the binary leaves an already adopted client valid and managed; it does not remove a client, scope, role, or membership.
