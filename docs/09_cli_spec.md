...
The CLI exposes a `user` command group for global account lifecycle operations. Tenant access belongs to Code Admin Identity V2.

`user create` creates a global directory user with an administrator-supplied temporary password. This maps to `POST /v1/admin/identity/directory/users`, uses the admin token in the Authorization header, and never accepts the password in process arguments or prints it after submission. The normal command prompts without echo; automation may pipe a secret producer into `--temporary-password-stdin`.
...
The CLI includes these command groups: `init`, `auth`, `token`, `tenant`, `user`, `role`, `client`, `apikey`, `jwks`, `saml`, `revoke`, and `config`. The `saml` group contains `metadata`, `idp register`, `idp show`, and `keys rotate` subcommands.
