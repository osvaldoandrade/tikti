...
|---|---|---|---|
| Spoofing | An attacker posts a valid response from the attacker's login into a victim browser. | Tikti still requires the state cookie on the same-origin repost and validates `InResponseTo`, issuer, audience, destination, signature, and subject confirmation. | Missing-cookie retry test rejects; production audit must record `accept` only after the second POST. |
