...
// loadWrappingFixture reads a test XML fixture from testdata/ and parses it
// into an etree.Document for signature wrapping tests.
func loadWrappingFixture(t *testing.T, name string) *etree.Document {
	t.Helper()
...
func TestWrap_ExtendedWrap(t *testing.T) {
	doc := loadWrappingFixture(t, "attack_wrap_extended.xml")
...
func TestWrap_SiblingUnsigned(t *testing.T) {
	doc := loadWrappingFixture(t, "attack_wrap_sibling.xml")
	signedRoot := firstAssertion(t, doc)
	refs := []string{"#_a1"}
...
func TestWrap_DuplicateID(t *testing.T) {
	doc := loadWrappingFixture(t, "attack_wrap_duplicate_id.xml")
	signedRoot := firstAssertion(t, doc)
	refs := []string{"#_dup"}
...
func TestWrap_OutOfScopeReference(t *testing.T) {
	doc := loadWrappingFixture(t, "attack_wrap_out_of_scope.xml")
	signedRoot := firstAssertion(t, doc)
...
func TestWrap_AncestorMove(t *testing.T) {
	doc := loadWrappingFixture(t, "attack_wrap_ancestor_move.xml")
...
func TestWrap_ValidAssertion_Accept(t *testing.T) {
	doc := loadWrappingFixture(t, "valid_assertion.xml")
	signedRoot := firstAssertion(t, doc)
	refs := []string{"#_a1"}
